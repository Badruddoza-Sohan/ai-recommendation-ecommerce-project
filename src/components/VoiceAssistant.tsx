...
export function VoiceAssistant() {
  ...
  const showExpandedVoiceUi = isActive || state === "STARTING";
  const showListeningStatus = state === "LISTENING" && !currentLiveTranscript && sttServiceStatus !== "retrying";
  const showTranscript = (state === "LISTENING" || state === "PROCESSING") && Boolean(currentLiveTranscript);
  const showProcessingStatus = state === "PROCESSING";
  const showAssistantResponse = state === "SPEAKING" && Boolean(response);
  ...
  return (
    <>
      ...
      {/* Active voice interaction panel: one state at a time, with separated areas. */}
      {showExpandedVoiceUi && (
        <div className="fixed bottom-24 right-4 z-[140] w-[min(22rem,calc(100vw-2rem))] rounded-2xl border border-slate-700 bg-slate-900/95 p-3.5 text-xs text-white shadow-2xl backdrop-blur-md animate-in fade-in zoom-in-95 sm:bottom-28 sm:right-6">
          {sttServiceStatus === "retrying" && (
            <div className="flex items-center gap-2 rounded-xl border border-amber-500/60 bg-amber-950/70 px-3 py-2 text-amber-200">
              <AlertTriangle className="h-4 w-4 shrink-0 text-amber-400" />
              <span>Voice service reconnecting... Attempt {networkRetryCount}/5</span>
            </div>
          )}

          {showListeningStatus && (
            <div className="flex items-center gap-2 rounded-xl border border-teal-500/50 bg-teal-950/50 px-3 py-2.5 text-teal-200">
              <Radio className="h-4 w-4 shrink-0 animate-pulse text-teal-400" />
              <span className="font-semibold">Listening...</span>
            </div>
          )}

          {showTranscript && (
            <div className="rounded-xl border border-red-500/40 bg-red-950/30 px-3 py-2.5">
              <p className="mb-1 text-[10px] font-bold uppercase tracking-wider text-red-300">You</p>
              <p className="font-semibold leading-relaxed text-red-100">{currentLiveTranscript}</p>
            </div>
          )}

          {showProcessingStatus && (
            <div className="mt-2 flex items-center gap-2 rounded-xl border border-indigo-500/40 bg-indigo-950/40 px-3 py-2 text-indigo-200">
              <Radio className="h-4 w-4 shrink-0 animate-pulse text-indigo-400" />
              <span className="font-semibold">Processing...</span>
            </div>
          )}

          {showAssistantResponse && (
            <div className="rounded-xl border border-purple-500/40 bg-purple-950/30 px-3 py-2.5">
              <p className="mb-1 flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-purple-300">
                <Volume2 className="h-3.5 w-3.5" /> Dev
              </p>
              <p className="leading-relaxed text-purple-100">{response}</p>
            </div>
          )}
        </div>
      )}
      ...
      <div className="fixed bottom-6 right-4 z-[140] flex items-center gap-2 sm:bottom-8 sm:right-6">
        <button
