...
export default function Products() {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [initialSearch, initialCategory, initialFeatured, initialTrending]);
  ...
  useEffect(() => {
    ...
    setVoiceProducts(sortedProducts);
    ...
    if (query && lastAnnouncedSearchRef.current !== query) {
      ...
      if (sortedProducts.length > 0) {
        const topFew = sortedProducts.slice(0, 4).map((p, index) => ({
          name: p.name,
          price: formatCurrency(p.price),
          index: index + 1
        }));
        const nextOptionStart = topFew.length + 1;
        const nextOptionEnd = Math.min(nextOptionStart + 3, sortedProducts.length);
        const details = topFew
          .map((p) => `Option ${p.index}: ${p.name} for ${p.price}`)
          .join(". ");
        const continuationHint =
          sortedProducts.length > topFew.length
            ? `Say 'continue' to hear option ${nextOptionStart}${nextOptionEnd > nextOptionStart ? ` to ${nextOptionEnd}` : ""}, or say 'Select 1' or 'Option 1' to choose and proceed to Voice Checkout.`
            : `Say 'Select 1' or 'Option 1' to choose and proceed to Voice Checkout.`;
        const message = `Found ${sortedProducts.length} items for ${query}. ${details}. ${continuationHint}`;
        if (typeof sessionStorage !== "undefined") {
          sessionStorage.setItem("voice_announced_count", String(topFew.length));
        }
        window.dispatchEvent(
          new CustomEvent("voice-assistant-announce", {
            detail: {
              message,
              spokenProductCount: topFew.length,
            },
          })
        );
      } else {
  ...
  const loadedCount = sortedProducts.length;
  const totalCount = data?.total ?? loadedCount;
  const totalPages = Math.max(1, Math.ceil(loadedCount / PRODUCTS_PER_PAGE));
  ...
  const firstResult = loadedCount === 0 ? 0 : pageStartIndex + 1;
  const lastResult = loadedCount === 0 ? 0 : Math.min(pageStartIndex + PRODUCTS_PER_PAGE, loadedCount);
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 animate-fade-up">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-6 p-2 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-md">
      ...
      {isLoading ? (
        ...
        >
          {paginatedProducts.map((product, index) => {
            ...
            const stockQuantity = Math.max(0, Number((product as any).quantity ?? (product as any).stock ?? 0));
            const stockStatus = stockQuantity === 0
              ? { label: "Out of Stock", className: "bg-rose-50 text-rose-700 dark:bg-rose-950/50 dark:text-rose-300 border-rose-200/70 dark:border-rose-900/50" }
              : stockQuantity <= 5
                ? { label: `Only ${stockQuantity} left`, className: "bg-amber-50 text-amber-700 dark:bg-amber-950/50 dark:text-amber-300 border-amber-200/70 dark:border-amber-900/50" }
                : { label: "In Stock", className: "bg-emerald-50 text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-300 border-emerald-200/70 dark:border-emerald-900/50" };
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 space-y-1.5 w-full">
                    ...
                    <span className={`inline-flex w-fit items-center rounded-full border px-2 py-0.5 text-[10px] font-bold ${stockStatus.className}`}>
                      {stockStatus.label}
                    </span>
                  ...
                  <div className="flex sm:flex-col items-center sm:items-end justify-between w-full sm:w-auto gap-3 shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-100 dark:border-slate-800">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className={`px-4 py-2.5 rounded-xl font-bold text-xs flex items-center gap-2 transition-all ${stockQuantity === 0
                          ? "cursor-not-allowed bg-slate-200 text-slate-500 dark:bg-slate-700 dark:text-slate-400"
                          : "cursor-pointer bg-indigo-600 text-white shadow-md hover:bg-indigo-700 hover:shadow-indigo-500/25"
                          }`}
                      >
            ...
            return (
              ...
              >
                <div>
                  ...
                  <div className="p-3 sm:p-4 flex flex-col gap-1.5">
                    <div className="flex items-center justify-between text-[11px] font-bold">
                      <span className="inline-flex max-w-[55%] items-center truncate rounded-full border border-indigo-200/70 bg-indigo-50 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-indigo-700 dark:border-indigo-800/70 dark:bg-indigo-950/50 dark:text-indigo-300">
                ...
                <div className="p-3 sm:p-4 mt-auto flex items-end justify-between gap-2 border-t border-slate-100 dark:border-slate-800/80 pt-2.5">
                  <div className="min-w-0">
                    <div className="text-lg sm:text-xl font-black text-slate-900 dark:text-white truncate">
                      {formatCurrency(product.price)}
                    </div>
                    {product.comparePrice && (
                      <span className="text-[11px] text-slate-400 dark:text-slate-500 line-through font-medium block truncate">
                        {formatCurrency(product.comparePrice)}
                      </span>
                    )}
                  </div>
                  ...
                  <button
                    onClick={() => addToCart.mutate({ productId: product.id })}
                    disabled={stockQuantity === 0}
                    className={`px-3.5 py-2 rounded-xl font-bold text-xs flex items-center gap-1.5 transition-all shrink-0 ${stockQuantity === 0
                      ? "cursor-not-allowed bg-slate-200 text-slate-500 dark:bg-slate-700 dark:text-slate-400"
                      : "cursor-pointer bg-indigo-600 text-white shadow-md shadow-indigo-600/20 hover:bg-indigo-700 hover:shadow-indigo-600/35"
                      }`}
      ...
      {!isLoading && !error && loadedCount > 0 && (
        <div className="mt-8 flex flex-col items-center justify-between gap-4 border-t border-slate-200 pt-5 dark:border-slate-800 sm:flex-row">
          <p className="text-sm font-medium text-slate-500 dark:text-slate-400">
            Showing {firstResult}&ndash;{lastResult} of {totalCount} products
          </p>
          {totalPages > 1 && (
            <nav className="flex flex-wrap items-center justify-center gap-1.5" aria-label="Product pagination">
              <button
                type="button"
                onClick={() => setCurrentPage((page) => Math.max(1, page - 1))}
                disabled={currentPage === 1}
                className="inline-flex items-center gap-1 rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-40 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"
              >
              ...
              {Array.from({ length: totalPages }, (_, index) => index + 1).map((page) => (
                <button
                  ...
                  className={`min-w-9 rounded-xl px-3 py-2 text-xs font-bold transition ${currentPage === page ? "bg-indigo-600 text-white shadow-sm" : "border border-slate-200 text-slate-700 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"}`}
                >
                  {page}
                </button>
              ))}
