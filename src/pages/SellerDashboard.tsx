...
import { getAttributeSchema, getMainCategoryForName, PRODUCT_ATTRIBUTE_SCHEMAS } from "@/lib/productAttributeSchemas";
import {
  Package,
  ShoppingBag,
  TrendingUp,
...
function SellerDashboardContent() {
  ...
  const [form, setForm] = useState({ name: "", description: "", shortDescription: "", price: "", comparePrice: "", deliveryFeeInsideDhaka: "", deliveryFeeOutsideDhaka: "", categoryId: "", mainCategory: "", subcategory: "", attributes: {} as Record<string, string>, status: "draft" as "active" | "draft" | "archived", sku: "", quantity: "0", lowStockThreshold: "5", warehouseLocation: "", imageUrl: "", images: [""] as string[], keywords: "", sizeType: "", variants: [] as { size: string, color: string, quantity: number }[] });
  ...
  const { data: sellerStatus, isLoading: sellerStatusLoading } = trpc.seller.status.useQuery(undefined, {
    enabled: !!authUser && (authUser as any).role === "seller",
  });
  const sellerApproved = sellerStatus?.status === "approved";
  const { data: dashboard, isLoading } = trpc.seller.dashboard.useQuery(undefined, {
    enabled: sellerApproved,
  ...
  const { data: sellerProducts, isLoading: sellerProductsLoading, isError: sellerProductsError, refetch: refetchSellerProducts } = trpc.seller.listProducts.useQuery(undefined, {
    enabled: sellerApproved,
  ...
  const createProduct = trpc.seller.createProduct.useMutation({
    onSuccess: async () => {
      ...
      setForm({ name: "", description: "", shortDescription: "", price: "", comparePrice: "", deliveryFeeInsideDhaka: "", deliveryFeeOutsideDhaka: "", categoryId: "", mainCategory: "", subcategory: "", attributes: {}, status: "draft", sku: "", quantity: "0", lowStockThreshold: "5", warehouseLocation: "", imageUrl: "", images: [""], keywords: "", sizeType: "", variants: [] });
  ...
  if (sellerStatusLoading || (sellerApproved && isLoading)) {
    return (
  ...
  if (!sellerApproved) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-16 text-center">
        <div className="rounded-3xl border border-amber-200 bg-amber-50 p-8 shadow-sm dark:border-amber-900/50 dark:bg-amber-950/20">
          <Clock className="mx-auto h-10 w-10 text-amber-500" />
          <h2 className="mt-4 text-2xl font-bold text-slate-900 dark:text-white">Seller application under review</h2>
          <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">Your seller account must be approved by an administrator before products, inventory, orders, and other seller features become available.</p>
        </div>
      </div>
    );
  }
  ...
  const handleEditProduct = (product: any) => {
    ...
    try {
      if (product.attributes) {
        ...
        parsedAttributes = Object.fromEntries(Object.entries(attr).filter(([key]) => key !== "sizeType")) as Record<string, string>;
      }
    } catch(e) {}
    ...
    setForm({
      ...
      mainCategory: getMainCategoryForName(product.categoryName || ""),
      subcategory: parsedAttributes.subcategory || "",
  ...
  const submitProduct = async () => {
    ...
    if (!form.name || !form.price || !form.categoryId || (PRODUCT_ATTRIBUTE_SCHEMAS.some((schema) => schema.mainCategory === form.mainCategory) && !form.subcategory)) {
      toast.error("Please fill in all mandatory fields (Name, Price, Category, and Subcategory)");
      return;
    }
    ...
    const payload = {
      ...
      attributes: { ...form.attributes, subcategory: form.subcategory || "" },
  ...
  return (
    <div className="dashboard-shell relative flex min-h-screen min-w-0 bg-[#F4F6FC] dark:bg-slate-950/50">
      ...
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-50 shrink-0 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 transition-all duration-300 flex flex-col lg:static lg:min-h-screen lg:translate-x-0 ${sidebarOpen ? "w-64" : "w-20"} ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
        <div className="border-b border-slate-200 p-4 dark:border-slate-800">
          ...
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-[#E0E7FF] text-[#5B6DF8] dark:bg-indigo-900/30 dark:text-indigo-400"><Package className="h-6 w-6" /></div>
            {sidebarOpen && <div><h2 className="text-lg font-bold text-slate-900 dark:text-white">Seller Hub</h2><p className="text-xs text-slate-500 dark:text-slate-400">{dashboard?.seller?.businessName || "Seller Store"}</p></div>}
          </div>
        </div>
        <nav className="flex-1 space-y-2 p-4">
          <Link to="/" onClick={() => setIsMobileMenuOpen(false)} className="flex w-full items-center gap-3 rounded-xl px-3 py-3 text-slate-600 transition-all hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-slate-800/50 dark:hover:text-white">
          ...
          <SidebarLink icon={<Home className="h-5 w-5" />} label="Overview" active={activeMenu === "overview"} onClick={() => setActiveMenu("overview")} sidebarOpen={sidebarOpen} />
          {sidebarOpen && <p className="mt-6 px-3 py-2 text-xs font-semibold uppercase text-slate-500 dark:text-slate-400">Products</p>}
          <SidebarLink icon={<Package className="h-5 w-5" />} label="Products" active={activeMenu === "products"} onClick={() => setActiveMenu("products")} sidebarOpen={sidebarOpen} />
          <SidebarLink icon={<Boxes className="h-5 w-5" />} label="Inventory" active={activeMenu === "inventory"} onClick={() => setActiveMenu("inventory")} sidebarOpen={sidebarOpen} />
          {sidebarOpen && <p className="mt-6 px-3 py-2 text-xs font-semibold uppercase text-slate-500 dark:text-slate-400">Sales</p>}
          <SidebarLink icon={<ShoppingBag className="h-5 w-5" />} label="Orders" active={activeMenu === "orders"} onClick={() => setActiveMenu("orders")} sidebarOpen={sidebarOpen} badge={sellerOrders?.filter(o => o.status === "pending").length} />
          <SidebarLink icon={<BarChart3 className="h-5 w-5" />} label="Analytics" active={activeMenu === "analytics"} onClick={() => setActiveMenu("analytics")} sidebarOpen={sidebarOpen} />
          <SidebarLink icon={<DollarSignIcon className="h-5 w-5" />} label="Payouts" active={activeMenu === "payouts"} onClick={() => setActiveMenu("payouts")} sidebarOpen={sidebarOpen} />
          {sidebarOpen && <p className="mt-6 px-3 py-2 text-xs font-semibold uppercase text-slate-500 dark:text-slate-400">Customers</p>}
          <SidebarLink icon={<Star className="h-5 w-5" />} label="Reviews" active={activeMenu === "reviews"} onClick={() => setActiveMenu("reviews")} sidebarOpen={sidebarOpen} />
          <SidebarLink icon={<Users className="h-5 w-5" />} label="Customers" active={activeMenu === "customers"} onClick={() => setActiveMenu("customers")} sidebarOpen={sidebarOpen} />
          <SidebarLink icon={<MessageSquare className="h-5 w-5" />} label="Messages" active={activeMenu === "messages"} onClick={() => setActiveMenu("messages")} sidebarOpen={sidebarOpen} />
          {sidebarOpen && <p className="mt-6 px-3 py-2 text-xs font-semibold uppercase text-slate-500 dark:text-slate-400">Account</p>}
          <SidebarLink icon={<User className="h-5 w-5" />} label="Profile" active={activeMenu === "profile"} onClick={() => setActiveMenu("profile")} sidebarOpen={sidebarOpen} />
          <SidebarLink icon={<Settings className="h-5 w-5" />} label="Settings" active={activeMenu === "settings"} onClick={() => setActiveMenu("settings")} sidebarOpen={sidebarOpen} />
        </nav>
          ...
          <div className="mb-8 flex items-start justify-between gap-4 border-b border-slate-200 pb-5 dark:border-slate-800 lg:pb-6">
            <div className="flex items-center gap-4">
              <button
                ...
                aria-label="Open seller dashboard navigation"
              ...
              <div className="min-w-0">
                <p className="text-xs font-bold uppercase tracking-[0.18em] text-indigo-600 dark:text-indigo-400">Seller workspace</p>
                <h1 className="mt-1 text-2xl font-black tracking-tight text-slate-900 dark:text-white sm:text-3xl capitalize">{activeMenu === "overview" ? "Seller Overview" : activeMenu}</h1>
                <p className="mt-1 max-w-2xl text-sm leading-6 text-slate-500 dark:text-slate-400">
                  {activeMenu === "overview" ? `Manage ${dashboard?.seller?.businessName || "your store"} with a clear view of sales, products, and fulfillment.` : `Manage your ${activeMenu} effectively.`}
                </p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <ProfileMenu user={authUser as any} />
              <Link
                to="/seller/assistant"
                aria-label="Open seller AI assistant"
                className="flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-3 font-medium text-white transition-colors hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
              >
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      <div className="grid min-w-0 gap-6 xl:grid-cols-[1.1fr_0.9fr]">
        <div className="rounded-3xl border border-indigo-200 bg-indigo-600 p-5 text-white shadow-sm sm:p-7">
          <div className="flex items-center gap-2 text-sm font-bold text-indigo-100">
            <Sparkles className="h-4 w-4" />
            Store snapshot
          </div>
          <h2 className="mt-3 text-2xl font-black tracking-tight sm:text-3xl">Keep your store moving</h2>
          <p className="mt-2 max-w-xl text-sm leading-6 text-indigo-100">
            {stats?.pendingOrders ? `You have ${stats.pendingOrders} pending order${stats.pendingOrders === 1 ? "" : "s"} waiting for fulfillment.` : "Your latest store activity is ready to review."}
          </p>
          <div className="mt-6 grid grid-cols-2 gap-3 text-sm sm:max-w-md">
            <div className="rounded-2xl bg-white/10 p-3"><span className="block text-xs text-indigo-200">Lifetime sales</span><strong className="mt-1 block text-xl">{stats?.totalSales || 0}</strong></div>
            <div className="rounded-2xl bg-white/10 p-3"><span className="block text-xs text-indigo-200">Total reviews</span><strong className="mt-1 block text-xl">{dashboard?.reviews?.length || 0}</strong></div>
          </div>
        </div>

        <div className="min-w-0 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800 sm:p-6">
          <div className="mb-4 flex items-start justify-between gap-3">
            <div><h3 className="text-lg font-black tracking-tight text-slate-900 dark:text-white">Revenue overview</h3><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Monthly revenue from your real sales data.</p></div>
            <span className="rounded-full bg-indigo-50 px-2.5 py-1 text-xs font-bold text-indigo-700 dark:bg-indigo-950/40 dark:text-indigo-300">{revenueTrend}</span>
          </div>
          {monthlySalesChartData.length > 0 ? (
            <div className="h-[230px] w-full min-w-0 sm:h-[250px]">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={monthlySalesChartData} margin={{ top: 8, right: 8, left: -18, bottom: 0 }}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e5e7eb" />
                  <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fill: '#6b7280', fontSize: 11}} dy={8} />
                  <YAxis axisLine={false} tickLine={false} tick={{fill: '#6b7280', fontSize: 11}} tickFormatter={(value) => `${value} ৳`} width={58} />
                  <Tooltip cursor={{fill: 'rgba(79,70,229,0.06)'}} contentStyle={{borderRadius: '12px', border: '1px solid #e5e7eb', boxShadow: '0 8px 20px rgba(15,23,42,0.10)'}} formatter={(value) => [formatCurrency(Number(value)), "Revenue"]} />
                  <Bar dataKey="revenue" fill="#4F46E5" radius={[6, 6, 0, 0]} barSize={28} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          ) : (
            <div className="flex h-[230px] w-full items-center justify-center rounded-2xl border border-dashed border-slate-300 text-center text-sm text-slate-400 dark:border-slate-700 sm:h-[250px]">
              No revenue data yet
            </div>
          )}
        </div>
      ...
      <div className="grid min-w-0 grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-5">
        <StatCard
          ...
          trend={revenueTrend}
          trendData={monthlySalesChartData.length > 1 ? monthlySalesChartData.map(d => ({ v: d.revenue })) : undefined}
          color="yellow"
        />
        <StatCard
          ...
          {sellerOrders && sellerOrders.length > 0 ? (
            <div className="space-y-3">
              {sellerOrders.slice(0, 5).map((order: any) => (
                <div
                  key={order.id}
                  className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] items-center gap-3 rounded-2xl border-b border-slate-100 px-2 py-3 last:border-0 dark:border-slate-700 sm:flex"
                >
                  <div className="min-w-0">
                    <p className="truncate text-sm font-bold text-slate-900 dark:text-white">{order.orderNumber}</p>
                    <p className="text-xs text-slate-500">
                      {order.createdAt ? new Date(order.createdAt).toLocaleDateString() : ""}
                    </p>
                  </div>
                  <span className={`justify-self-end rounded-full px-2.5 py-1 text-xs font-bold ${
                  ...
                  <span className="col-start-1 text-sm font-bold text-slate-900 dark:text-white sm:col-auto">
            ...
            <div className="rounded-2xl border border-dashed border-slate-300 px-5 py-8 text-center dark:border-slate-700">
              ...
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">New customer orders will appear here.</p>
            </div>
          )}
        </div>

      {/* Quick Actions */}
      <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800 sm:p-6">
        <h2 className="text-lg font-black tracking-tight text-slate-900 dark:text-white">Quick actions</h2>
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Jump into the tools you use most.</p>
...
function ProductsSection({
  ...
  const selectedSchema = getAttributeSchema(form.mainCategory, form.subcategory);
  const mainCategoryNames = PRODUCT_ATTRIBUTE_SCHEMAS.map((schema) => schema.mainCategory);
  const subcategoryNames = PRODUCT_ATTRIBUTE_SCHEMAS.find((schema) => schema.mainCategory === form.mainCategory)
    ? Object.keys(PRODUCT_ATTRIBUTE_SCHEMAS.find((schema) => schema.mainCategory === form.mainCategory)!.subcategories)
    : [];
  ...
  return (
    <div className="space-y-6">
      {sellerProductsError && (
        <div className="flex flex-col gap-3 rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm text-rose-700 dark:border-rose-900/50 dark:bg-rose-950/20 dark:text-rose-300 sm:flex-row sm:items-center sm:justify-between">
          <span>Products could not be loaded. Please try again.</span>
          <button type="button" onClick={() => refetchSellerProducts()} className="font-bold underline">Retry</button>
        </div>
      )}
      {sellerProductsLoading && <div className="rounded-2xl border border-indigo-100 bg-indigo-50 p-4 text-sm font-semibold text-indigo-700 dark:border-indigo-900/50 dark:bg-indigo-950/20 dark:text-indigo-300">Loading your products...</div>}
      {/* Header Card */}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const category = (categories || []).find((cat: any) => cat.name.toLowerCase() === mainCategory.toLowerCase());
                    setForm({ ...form, mainCategory, subcategory: "", attributes: {}, categoryId: category ? String(category.id) : "" });
                  }}
                ...
                >
                  <option value="">Step 1: Main category *</option>
                  {mainCategoryNames.map((name) => <option key={name} value={name}>{name}</option>)}
                  {(categories || []).filter((cat: any) => !mainCategoryNames.includes(cat.name)).map((cat: any) => (
                    <option key={cat.id} value={cat.name}>{cat.name}</option>
                  ))}
                </select>
                <select
                  value={form.subcategory}
                  disabled={!form.mainCategory || subcategoryNames.length === 0}
                  onChange={(e) => setForm({ ...form, subcategory: e.target.value, attributes: {} })}
                  className="rounded-xl border border-slate-300 px-4 py-2.5 dark:bg-slate-700 dark:border-slate-600 dark:text-white focus:ring-2 focus:ring-indigo-500 text-sm font-medium disabled:opacity-50"
                >
                  <option value="">Step 2: Subcategory *</option>
                  {subcategoryNames.map((name) => <option key={name} value={name}>{name}</option>)}
                </select>
              </div>
              <p className="text-[11px] text-slate-500">Choose a main category, then a product type to load the right specification fields.</p>
            </div>

            {selectedSchema.length > 0 && (
              <div className="md:col-span-2 rounded-2xl border border-indigo-100 bg-indigo-50/50 p-4 dark:border-indigo-900/50 dark:bg-indigo-950/20">
                <div className="mb-3">
                  <h4 className="text-xs font-bold uppercase tracking-wide text-indigo-700 dark:text-indigo-300">Step 3: {form.subcategory} specifications</h4>
                  <p className="text-[11px] text-slate-500 mt-1">These details appear in the product specification table.</p>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {selectedSchema.map((field) => (
                    <label key={field.key} className="flex flex-col gap-1 text-xs font-semibold text-slate-700 dark:text-slate-200">
                      {field.label}
                      <input
                        value={form.attributes[field.key] || ""}
                        onChange={(e) => setForm({ ...form, attributes: { ...form.attributes, [field.key]: e.target.value } })}
                        className="rounded-lg border border-slate-300 px-3 py-2 font-normal dark:border-slate-600 dark:bg-slate-700 dark:text-white"
...
function OrdersSection({ sellerOrders }: any) {
  ...
  const updateOrderShippingDetails = trpc.seller.updateOrderShippingDetails.useMutation({
    onSuccess: async () => {
      ...
      await utils.seller.getOrders.invalidate();
      await utils.seller.dashboard.invalidate();
    },
    onError: (err) => {
      setIsOrderEditSubmitting(false);
...
function MessagesSection({ messages: _initialMessages }: any) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white dark:bg-slate-800/95 rounded-2xl border border-slate-200/80 dark:border-slate-700/80 shadow-lg overflow-hidden flex flex-col md:flex-row min-h-[620px]">
        {/* ==================================================================== */}
...
function StatCard({
  ...
  const colorMap: Record<string, string> = {
    ...
    purple: "border-violet-200 bg-violet-50 text-violet-950 dark:border-violet-900/60 dark:bg-violet-950/30 dark:text-violet-100",
    cyan: "border-cyan-200 bg-cyan-50 text-cyan-950 dark:border-cyan-900/60 dark:bg-cyan-950/30 dark:text-cyan-100",
    red: "border-rose-200 bg-rose-50 text-rose-950 dark:border-rose-900/60 dark:bg-rose-950/30 dark:text-rose-100",
    blue: "border-blue-200 bg-blue-50 text-blue-950 dark:border-blue-900/60 dark:bg-blue-950/30 dark:text-blue-100",
    slate: "border-slate-200 bg-slate-50 text-slate-950 dark:border-slate-700 dark:bg-slate-900/60 dark:text-slate-100",
    yellow: "border-yellow-200 bg-yellow-50 text-yellow-950 dark:border-yellow-900/60 dark:bg-yellow-950/30 dark:text-yellow-100",
  ...
  const bgClass = alert ? "border-rose-200 bg-rose-50 text-rose-950 dark:border-rose-900/60 dark:bg-rose-950/30 dark:text-rose-100" : colorMap[color] || colorMap.indigo;

  return (
    <div className={`${bgClass} relative flex min-h-[142px] flex-col justify-between overflow-hidden rounded-2xl border p-4 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md sm:p-5`}>
      <div className="relative z-10 flex items-start justify-between gap-3">
        <div className="flex flex-col">
          <p className="text-[11px] font-bold uppercase tracking-wide opacity-70">{label}</p>
          <div className="flex items-baseline gap-2">
            <p className="mt-2 text-xl font-black tracking-tight leading-tight sm:text-2xl">{value}</p>
            {trend && (
              <span className="rounded-md bg-white/70 px-1.5 py-0.5 text-xs font-bold opacity-80 dark:bg-slate-900/50">
                {trend}
              </span>
            )}
          </div>
      ...
      {trendData && trendData.length > 0 && (
        <div className="absolute bottom-2 left-4 right-4 h-9 opacity-45">
          <ResponsiveContainer width="100%" height="100%">
