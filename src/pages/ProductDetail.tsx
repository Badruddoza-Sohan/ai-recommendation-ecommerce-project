  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 animate-in fade-in duration-700">
      ...
      <div className="glass-card rounded-3xl p-6 sm:p-8 mb-10 animate-fade-up" style={{ animationDelay: '200ms' }}>
        ...
        {/* Attributes */}
        {Object.entries(parsedAttributes).some(([key, value]) => key !== "sizeType" && value !== "") && (
          <div className="mt-6">
              ...
              <table className="w-full text-sm">
                <tbody>
                  {Object.entries(parsedAttributes)
                    .filter(([key, value]) => key !== "sizeType" && value !== "")
                    .map(([key, value], index) => (
                    <tr key={key} className={index % 2 === 0 ? "bg-slate-50/70 dark:bg-slate-800/60" : "bg-white dark:bg-slate-900/30"}>
                      <th className="w-1/3 px-4 py-3 text-left font-medium text-slate-500 capitalize">{key.replace(/([A-Z])/g, " $1")}</th>
                      <td className="px-4 py-3 font-semibold text-slate-900 dark:text-white">{String(value)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
