...
function isExplicitPauseCommand(value: string): boolean {
  const normalized = value
    .toLowerCase()
    .replace(/[.,!?]/g, " ")
    .replace(/\s+/g, " ")
    .trim();
  return /^(?:pause|sleep|dev pause|dev sleep|hey dev pause|hey dev sleep)$/.test(normalized);
}
...
export function useVoiceAssistantEngine({
  ...
  const startSttListening = useCallback(() => {
    ...
    if (!sttProviderRef.current) {
      // Deepgram Integration Hook
      const deepgramKey = import.meta.env.VITE_DEEPGRAM_API_KEY;
      if (deepgramKey) {
        console.log("[VoiceEngine] VITE_DEEPGRAM_API_KEY detected. Deepgram STT adapter will be used (Coming Soon). Falling back to BrowserSTT.");
        // sttProviderRef.current = new DeepgramSttProvider({ key: deepgramKey, language });
      }
    ...
    provider.onResult = (result: SttResult) => {
      ...
      // ── Explicit Pause/Sleep Check ─────────────────────────────────────────
      const isPauseCommand = isExplicitPauseCommand(spokenText);
      if (isPauseCommand && (stateRef.current !== "OFF" || isSpeakingRef.current)) {
        console.log("[VoiceEngine] 💤 Explicit pause/sleep command detected:", spokenText);
        if (sttProviderRef.current?.isActive()) sttProviderRef.current.abort();
        try {
          window.speechSynthesis.cancel();
        } catch {}
      ...
      const isStopWord = /\b(stop|quiet|be quiet|shut up|silence|cancel|thamo|thamun|bondho koro|chup)\b/i.test(spokenText);
  ...
  // ── Deactivate Assistant (explicit user pause; retain passive wake detection) ──
