...
import { authenticateRequest, createOAuthCallbackHandler } from "./auth/auth";
...
app.get("/payment-callback", (c) => {
  ...
  if (!fs.existsSync(indexPath)) {
    return c.text("Frontend entry point is unavailable.", 503);
  }
...
app.post("/api/upload", async (c) => {
  try {
    ...
    if (!["image/jpeg", "image/png", "image/webp"].includes(file.type)) return c.json({ error: "Only JPG, PNG, and WebP images are supported" }, 415);
    ...
    if (buffer.byteLength > 10 * 1024 * 1024) return c.json({ error: "Image size must be 10 MB or less" }, 413);
    const bytes = new Uint8Array(buffer);
    const isJpeg = bytes[0] === 0xff && bytes[1] === 0xd8 && bytes[2] === 0xff;
    const isPng = bytes[0] === 0x89 && bytes[1] === 0x50 && bytes[2] === 0x4e && bytes[3] === 0x47;
    const isWebp = bytes[0] === 0x52 && bytes[1] === 0x49 && bytes[2] === 0x46 && bytes[3] === 0x46 && bytes[8] === 0x57 && bytes[9] === 0x45 && bytes[10] === 0x42 && bytes[11] === 0x50;
    if ((file.type === "image/jpeg" && !isJpeg) || (file.type === "image/png" && !isPng) || (file.type === "image/webp" && !isWebp)) return c.json({ error: "The uploaded file is not a valid image" }, 415);
...
app.post("/api/ai/stream-text", async (c) => {
  ...
  return streamText(c, async (stream) => {
    try {
      ...
      const errorMsg = err.message?.includes("429") || err.message?.includes("Max retries") 
