...
import { orders, orderItems, carts, cartItems, products, sellers, notifications, productImages, users, productVariants, inventory } from "@db/schema";
...
async function sendInvoiceIfNeeded(db: ReturnType<typeof getDb>, orderId: number) {
  try {
    const orderRows = await db.select().from(orders).where(eq(orders.id, orderId)).limit(1);
    const order = orderRows[0];
    if (!order || !order.userId || (order.notes || "").includes("[INVOICE_EMAIL_SENT]")) return;

    const userRows = await db.select().from(users).where(eq(users.id, order.userId)).limit(1);
    const user = userRows[0];
    if (!order.shippingEmail && !user?.email) return;

    const items = await db
      .select({ name: products.name, quantity: orderItems.quantity, unitPrice: orderItems.unitPrice })
      .from(orderItems)
      .leftJoin(products, eq(products.id, orderItems.productId))
      .where(eq(orderItems.orderId, orderId));

    // Reserve the marker before sending so retries or duplicate callbacks cannot send twice.
    await db.update(orders).set({ notes: `${order.notes || ""} [INVOICE_EMAIL_SENT]` }).where(eq(orders.id, orderId));
    await sendOrderInvoiceEmail({
      to: order.shippingEmail || user.email,
      customerName: order.shippingFullName || user.name || "Customer",
      orderNumber: order.orderNumber,
      orderDate: order.createdAt,
      items: items.map((item) => ({ name: item.name || "Product", quantity: item.quantity, unitPrice: item.unitPrice })),
      ...
      shippingCity: order.shippingCity || order.shippingDistrict || "",
      shippingCountry: order.shippingCountry || "Bangladesh",
      shippingPostalCode: order.shippingPostalCode || "",
      invoiceUrl: `${env.appUrl.replace(/\/$/, "")}/invoice/${order.id}`,
    });
  } catch (error) {
    console.error(`[Order] Invoice email could not be prepared for order ${orderId}:`, error);
  }
}
...
async function createOrderFromItems(
  ...
  userId: number | null,
  items: Array<any>,
  input: { shippingFullName?: string; shippingEmail?: string; shippingPhone?: string; shippingAddress: string; shippingCity: string; shippingDistrict?: string; shippingCountry: string; shippingPostalCode: string; paymentMethod: string },
  cartId?: number,
) {
  ...
  const shippingAmount = items.reduce((sum, item) => {
    const fee = deliveryZone === "inside_dhaka" ? item.deliveryFeeInsideDhaka ?? 0 : item.deliveryFeeOutsideDhaka ?? 0;
  ...
  const orderResult = await db.insert(orders).values({
    orderNumber, userId: userId || 1, totalAmount, taxAmount, shippingAmount,
    status: "pending", sellerStatus: "pending", estimatedDeliveryDays: estimatedDays,
    estimatedDeliveryDate: Date.now() + estimatedDays * 86400000,
    paymentStatus: input.paymentMethod === "cod" ? "pending" : "paid",
    paymentMethod: input.paymentMethod, paymentReference, shippingAddress: input.shippingAddress,
    shippingFullName: input.shippingFullName || "", shippingEmail: input.shippingEmail || "", shippingPhone: input.shippingPhone || "", shippingCity: input.shippingCity,
    shippingDistrict: input.shippingDistrict || input.shippingCity, shippingCountry: input.shippingCountry,
    shippingPostalCode: input.shippingPostalCode, trackingNumber,
    notes: buildDemoPaymentNote(paymentReference, trackingNumber),
  });
  ...
  for (const item of items) {
    ...
    await db.insert(orderItems).values({
      orderId, productId: item.productId, sellerId, quantity: item.quantity,
      unitPrice: item.price || 0, totalPrice: (item.price || 0) * item.quantity,
    ...
    if (item.attributes) {
      try {
        ...
        if (attributes.size || attributes.color) {
          const variants = await db.select().from(productVariants).where(eq(productVariants.productId, item.productId));
          const match = variants.find((variant: any) => (!attributes.size || variant.size === attributes.size) && (!attributes.color || variant.color === attributes.color));
          if (match) {
            await db.update(productVariants).set({ quantity: Math.max(0, match.quantity - item.quantity) }).where(eq(productVariants.id, match.id));
          }
        }
      } catch {
        // Preserve order placement when legacy attribute JSON is malformed.
      }
    }
    const inventoryRows = await db.select().from(inventory).where(eq(inventory.productId, item.productId)).limit(1);
    if (inventoryRows[0]) {
      await db.update(inventory).set({ quantity: Math.max(0, inventoryRows[0].quantity - item.quantity) }).where(eq(inventory.id, inventoryRows[0].id));
    }
  }
  try {
    await notifySellersForOrder(Array.from(sellerIds), orderNumber, totalAmount, items.length);
    if (userId) {
      await db.insert(notifications).values({ userId, title: `Order Placed Successfully #${orderNumber}`, message: `Your order #${orderNumber} for BDT ${totalAmount.toLocaleString()} has been placed.`, type: "order", isRead: 0, link: "/dashboard?tab=orders" });
      await notifyAdmins({ title: `🛒 New Order #${orderNumber}`, message: `Total BDT ${totalAmount.toLocaleString()} order placed by customer.`, type: "order", link: "/admin?tab=orders" });
    }
  } catch (notificationError) {
    console.error("[Order] Order created but notification delivery failed:", notificationError);
  }
  if (cartId) await db.delete(cartItems).where(eq(cartItems.cartId, cartId));
  await sendInvoiceIfNeeded(db, orderId);
  return { success: true, orderId, orderNumber, paymentReference, trackingNumber, totalAmount, status: "pending" };
}
...
export const orderRouter = createRouter({
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.standaloneItem) {
        const item = await db
          ...
          .from(products)
          .where(eq(products.id, input.standaloneItem.productId))
          .limit(1);
        if (!item[0]) throw new Error("Product not found");
        return createOrderFromItems(db, userId, [{
          ...item[0],
          quantity: input.standaloneItem.quantity,
          attributes: input.standaloneItem.attributes ? JSON.stringify(input.standaloneItem.attributes) : null,
        }], input);
      }
      ...
      return createOrderFromItems(db, userId, items, input, cartId);
    }),
  ...
  voiceCreate: publicQuery
    ...
    .mutation(async ({ input }) => {
      ...
      const item = [{ productId: input.productId, quantity: input.quantity, price: product[0].price, sellerId: product[0].sellerId, deliveryFeeInsideDhaka: product[0].deliveryFeeInsideDhaka, deliveryFeeOutsideDhaka: product[0].deliveryFeeOutsideDhaka, attributes: null }];
      return createOrderFromItems(db, null, item, { shippingAddress: input.shippingAddress, shippingCity: "Dhaka", shippingCountry: "Bangladesh", shippingPostalCode: "1200", paymentMethod: "cod" });
    }),
...
// REMOVE
      ...
      for (const item of items) {
        ...
        if (item.attributes) {
          try {
            ...
            if (attr.size || attr.color) {
               const variants = await db.select().from(productVariants).where(eq(productVariants.productId, item.productId));
               const match = variants.find((v: any) => (!attr.size || v.size === attr.size) && (!attr.color || v.color === attr.color));
               if (match) {
                 await db.update(productVariants).set({ quantity: Math.max(0, match.quantity - item.quantity) }).where(eq(productVariants.id, match.id));
               }
            }
        ...
        // Decrement overall inventory
        const inv = await db.select().from(inventory).where(eq(inventory.productId, item.productId)).limit(1);
        if (inv[0]) {
           await db.update(inventory).set({ quantity: Math.max(0, inv[0].quantity - item.quantity) }).where(eq(inventory.id, inv[0].id));
        }
      }
  ...
  */
  ...
  confirmReceived: authedQuery
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (orderRows[0].status !== "shipped") {
        throw new TRPCError({ code: "BAD_REQUEST", message: "Only shipped orders can be marked as received." });
      }

      await db
        .update(orders)
        .set({ status: "delivered", paymentStatus: "paid", updatedAt: Date.now() })
        .where(and(eq(orders.id, input.orderId), eq(orders.status, "shipped")));
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.standaloneItem) {
        items = await db.select({
          ...
          deliveryFeeInsideDhaka: products.deliveryFeeInsideDhaka,
          deliveryFeeOutsideDhaka: products.deliveryFeeOutsideDhaka,
        }).from(products).where(eq(products.id, input.standaloneItem.productId)).limit(1);
      ...
      if (items.length === 0) throw new Error("Product or cart is empty");
      ...
      const sslResponse = await initiateSSLCommerzPayment({
        ...
        cusName: input.shippingFullName || input.customerName || "Valued Customer",
        ...
        cusPhone: input.shippingPhone || input.customerPhone || "01700000000",
      ...
      // Only use fallback mock gateway in sandbox mode (non-live)
      ...
      // In live mode, if SSLCommerz fails to initiate payment, return error
      return { success: false, message: "Failed to initiate SSLCommerz payment. Please try again." };
    }),
  ...
  verifySSLCommerz: publicQuery
    ...
    .mutation(async ({ input }) => {
      ...
      if (!orderRows[0]) {
        return { success: false, message: "Order not found. Please check your transaction ID." };
      }

      const order = orderRows[0];
      const statusLower = (input.status || "").toLowerCase();
      const isSuccess = statusLower.includes("success") || statusLower === "completed" || statusLower === "valid";
      const isCancelled = statusLower.includes("cancel");
      const isFailed = statusLower.includes("fail") || statusLower.includes("error") || statusLower.includes("unattempted");
      ...
      if (isCancelled || isFailed) {
        ...
        const failureReason = isCancelled 
          ? "Payment was cancelled. Your order is still in your cart and ready to checkout again."
          : "Payment failed due to transaction error. Please try again with valid payment details.";

        // Notify user of payment failure
        if (order.userId) {
          await db.insert(notifications).values({
            userId: order.userId,
            title: `❌ Payment ${isCancelled ? "Cancelled" : "Failed"} #${order.orderNumber}`,
      ...
      if (!isSuccess) {
        return { 
          success: false, 
          message: "Invalid payment status. Unable to verify transaction.", 
          orderNumber: order.orderNumber 
        };
      }
      ...
      await sendInvoiceIfNeeded(db, order.id);
