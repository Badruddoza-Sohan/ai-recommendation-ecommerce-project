import { z } from "zod";
import { createRouter, publicQuery, authedQuery, approvedSellerQuery, approvedSellerMutation } from "./middleware";
import { TRPCError } from "@trpc/server";
import { getDb } from "./queries/connection";
import { sellers, products, productImages, orders, orderItems, categories, inventory, reviews, notifications, users, sellerPayouts, sellerMessages, productVariants } from "@db/schema";
import { eq, desc, and, sql, inArray } from "@db/mysql";
...
async function assertSellerOwnsOrder(orderId: number, sellerId: number) {
  const db = getDb();
  const ownedItem = await db
    ...
    .limit(1);
  if (!ownedItem[0]) {
    throw new TRPCError({ code: "FORBIDDEN", message: "You are not authorized to modify this order." });
  }
}

export const sellerRouter = createRouter({
  status: authedQuery.query(async ({ ctx }) => {
    const seller = await getSellerByUserId((ctx.user as any).id);
  ...
  listProducts: approvedSellerQuery.query(async ({ ctx }) => {
    const db = getDb();
    ...
    const productList = await db
      .select({
        ...
        attributes: (products as any).attributes,
        imageUrl: (productImages as any).imageUrl,
    ...
    const allVariants = await db.select().from(productVariants).where(inArray(productVariants.productId, productIds));
    ...
    return productList.map((product: any) => ({
      ...product,
      variants: allVariants.filter((v: any) => v.productId === product.id),
      images: allImages.filter((img: any) => img.productId === product.id).sort((a: any, b: any) => a.sortOrder - b.sortOrder),
    }));
  }),

  createProduct: approvedSellerMutation
    .input(
      z.object({
        name: z.string().min(1),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const baseSlug = input.name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "product";
      const existingSlug = await db.select({ id: products.id }).from(products).where(eq(products.slug, baseSlug)).limit(1);
      const slug = existingSlug[0] ? `${baseSlug}-${Date.now()}` : baseSlug;
      ...
      const productId = Number((insertResult as any)?.[0]?.insertId);
      ...
      if (input.variants && input.variants.length > 0) {
        totalQuantity = input.variants.reduce((sum, v) => sum + v.quantity, 0);
      ...
      const inserted = await db.select().from(products).where(eq(products.id, productId)).limit(1);
      return inserted[0];
    }),

  updateProduct: approvedSellerMutation
    .input(
      z.object({
        productId: z.number(),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.sizeType !== undefined) {
        let currentAttr = {};
        try { currentAttr = JSON.parse(existing[0].attributes || "{}"); } catch(e){}
        updatePayload.attributes = JSON.stringify({ ...currentAttr, sizeType: input.sizeType });
      }
      if (input.attributes !== undefined) {
        let currentAttr: Record<string, unknown> = {};
        try { currentAttr = JSON.parse(existing[0].attributes || "{}"); } catch(e){}
        updatePayload.attributes = JSON.stringify({ ...currentAttr, ...input.attributes });
      }
      ...
      if (input.variants !== undefined) {
        ...
        const invPayload: Record<string, unknown> = { quantity: totalQuantity };
        if (input.lowStockThreshold !== undefined) invPayload.lowStockThreshold = input.lowStockThreshold;
        if (input.warehouseLocation !== undefined) invPayload.warehouseLocation = input.warehouseLocation;
        
        await db.update(inventory)
          .set(invPayload)
          .where(eq(inventory.productId, input.productId));
      } else if (input.quantity !== undefined || input.lowStockThreshold !== undefined || input.warehouseLocation !== undefined) {
        const invPayload: Record<string, unknown> = {};
        if (input.quantity !== undefined) invPayload.quantity = input.quantity;
        if (input.lowStockThreshold !== undefined) invPayload.lowStockThreshold = input.lowStockThreshold;
        if (input.warehouseLocation !== undefined) invPayload.warehouseLocation = input.warehouseLocation;
  ...
  deleteProduct: approvedSellerMutation
  ...
  adjustInventory: approvedSellerMutation
    .input(
      z.object({
        productId: z.number(),
        delta: z.number().int(),
  ...
  updateProfile: approvedSellerMutation
    .input(
      z.object({
  ...
  dashboard: approvedSellerQuery.query(async ({ ctx }) => {
    const db = getDb();
    const userId = (ctx.user as any).id;
  ...
  updateOrderShippingDetails: approvedSellerMutation
    .input(z.object({
      orderId: z.number(),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await assertSellerOwnsOrder(input.orderId, seller.id);
      ...
      const orderRows = await db
        .select({
          ...
          shippingLandmark: orders.shippingLandmark,
        })
        .from(orders)
        .where(eq(orders.id, input.orderId))
        .limit(1);

      if (!orderRows[0]) throw new Error("Order not found for this seller");

      const sellerOwnsOrder = await db
        .select({ id: orderItems.id })
        .from(orderItems)
        .where(and(eq(orderItems.orderId, input.orderId), eq(orderItems.sellerId, seller.id)))
        .limit(1);
      if (!sellerOwnsOrder[0]) throw new Error("Order not found for this seller");

      const currentOrder = orderRows[0];

      const updateResult = await db.update(orders).set({
        shippingFullName: input.fullName !== undefined ? input.fullName.trim() : currentOrder.shippingFullName,
        shippingAddress: input.address !== undefined ? input.address.trim() : currentOrder.shippingAddress,
        shippingCity: input.district !== undefined ? input.district.trim() : currentOrder.shippingCity,
        shippingDistrict: input.district !== undefined ? input.district.trim() : currentOrder.shippingDistrict,
        shippingPostalCode: input.postalCode !== undefined ? input.postalCode.trim() : currentOrder.shippingPostalCode,
        shippingLandmark: input.landmark !== undefined ? input.landmark.trim() : currentOrder.shippingLandmark,
        lastEditedBy: input.editedBy,
        lastEditedAt: Date.now(),
        updatedAt: Date.now(),
      }).where(eq(orders.id, input.orderId));

      if (!updateResult) throw new Error("Order delivery details could not be saved");
      ...
      const savedOrder = await db
        .select({
          ...
          lastEditedAt: orders.lastEditedAt,
        })
        .from(orders)
        .where(eq(orders.id, input.orderId))
        .limit(1);

      if (!savedOrder[0]) throw new Error("Order delivery details could not be verified after saving");
      return { success: true, order: savedOrder[0] };
    }),

  getOrders: approvedSellerQuery.query(async ({ ctx }) => {
    const db = getDb();
    const userId = (ctx.user as any).id;
    ...
    const sellerOrders = await db
      .select({
        ...
        createdAt: (orders as any).createdAt,
        shippingFullName: (orders as any).shippingFullName,
        shippingAddress: (orders as any).shippingAddress,
        shippingCity: (orders as any).shippingCity,
        shippingDistrict: (orders as any).shippingDistrict,
        shippingCountry: (orders as any).shippingCountry,
        shippingPostalCode: (orders as any).shippingPostalCode,
        shippingLandmark: (orders as any).shippingLandmark,
        lastEditedBy: (orders as any).lastEditedBy,
        lastEditedAt: (orders as any).lastEditedAt,
      })
      .from(orders)
      .innerJoin(orderItems, eq(orderItems.orderId, orders.id))
      .where(eq(orderItems.sellerId, sellerId))
  ...
  // Payouts
  listPayouts: approvedSellerQuery.query(async ({ ctx }) => {
    const db = getDb();
    const userId = (ctx.user as any).id;
    const seller = await getSellerByUserId(userId);
    if (!seller) return [];
  ...
  requestPayout: approvedSellerMutation
    .input(
      z.object({
  ...
  // Messages
  listMessages: approvedSellerQuery.query(async ({ ctx }) => {
    const db = getDb();
    const userId = (ctx.user as any).id;
    const seller = await getSellerByUserId(userId);
    if (!seller) return [];
  ...
  replyMessage: approvedSellerMutation
    .input(
      z.object({
        messageId: z.number(),
  ...
  acceptOrder: approvedSellerMutation
  ...
  denyOrder: approvedSellerMutation
  ...
  handoverOrder: approvedSellerMutation
    .input(
      z.object({
        orderId: z.number(),
  ...
  suggestProductDetails: approvedSellerMutation
    .input(
      z.object({
...
Return ONLY valid JSON with keys: "shortDescription", "description", "keywords", "sku".`;
        ...
        if (!parsed.offline && !parsed.error) {
          return {
            ...
            source: apiKey ? "llm" : "ai",
          };
        }
      } catch (e) {
        console.error("[suggestProductDetails] AI generation error:", e);
      }
