...
import { handleDirectAction } from "../server/ai/routing/direct-action-router.ts";
  ...
  chat: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      try {
        ...
        const sessionId = input.sessionId || input.context?.sessionId || `session_${Date.now()}`;
        const directResult = await handleDirectAction({
          message: input.message,
          sessionId,
          userId: (ctx.user as any)?.id || input.context?.userId,
          domain: input.domain,
          context: input.context,
        });

        if (directResult) {
          console.log(`[RUNTIME-TRACE] [BrainRouter] DIRECT -> Intent: [${directResult.intent}], Domain: [${directResult.domain}]`);
          return {
            ok: true,
            ...
            structuredResponse: directResult.structuredResponse || null,
