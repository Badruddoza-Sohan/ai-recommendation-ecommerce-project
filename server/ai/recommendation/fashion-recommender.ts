...
`;
    ...
    const targetText = intent.targetItems.join(" ").toLowerCase();
    const requestedCategory = /(shoe|footwear|sneaker|loafer|sandal|boot)/i.test(targetText)
      ? ["shoe", "sneaker", "loafer", "sandal", "boot"]
      : /(pant|trouser|jean|bottom)/i.test(targetText)
        ? ["pant", "trouser", "jean"]
        : /(shirt|top|tee|blouse)/i.test(targetText)
          ? ["shirt", "t-shirt", "tee", "blouse"]
          : /(watch)/i.test(targetText)
            ? ["watch"]
            : undefined;
    ...
    const products = await this.searchEngine.search({
      keywords: keywords,
      requiredKeywords: requestedCategory,
