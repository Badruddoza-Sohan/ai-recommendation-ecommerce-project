...
export class CatalogSearchEngine {
  async search(criteria: SearchCriteria) {
    ...
    if (criteria.requiredKeywords && criteria.requiredKeywords.length > 0) {
      const requiredConditions = criteria.requiredKeywords.map((kw) => or(
        like(products.name, `%${kw.toLowerCase()}%`),
        like(products.tags, `%${kw.toLowerCase()}%`),
        like(products.attributes, `%${kw.toLowerCase()}%`)
      ));
